# -*- coding: utf-8 -*-
"""
纯 Python Turnstile 求解器（无浏览器 / 无 Node 依赖）。

移植自 sleep-reg 项目 protocol/turnstile.py（MIT License）：
  https://github.com/napnow/sleep-reg

用途：sentinel-runner.js(Node VM) 不可用或失败时，用纯 Python 计算
sentinel/req 返回的 turnstile.dx challenge，生成 t 字段。
原理：
  1. dx = base64(XOR(操作指令JSON, p))，p 为 req 请求体里的 requirements token
  2. 解码后得到指令队列（op map），用 Python 模拟 JS 虚拟机逐条执行
  3. 最终 result 即 turnstile token（base64）

注意：Cloudflare 更新 challenge 结构后可能失效；生产路径优先走
core/sentinel_runner.py（Node VM 跑官方 SDK），本模块仅作降级。
"""
import base64
import json
import random
