# -*- coding: utf-8 -*-
"""Mail.tm 公共临时邮箱客户端（零配置，开箱即用）。

参考 napnow/sleep-reg email_providers/duckmail.py（MIT）的 mail.tm API 用法：
    GET  /domains               -> {"hydra:member": [{"domain": ..., "isActive": ...}]}
    POST /accounts              -> 创建 {address, password}
    POST /token                 -> 换 {token}
    GET  /messages?page=1       -> 邮件列表（Bearer token）
    GET  /messages/{id}         -> 邮件详情（text/html）

定位：邮箱来源链 **末位兜底**。公共 mail.tm 域名被 OpenAI 风控标记较重，
注册成功率预期偏低，但零成本、零配置，适合在 outlook/generic_api/mailnest 等
来源全部领取失败时兜底，避免整批任务因无邮箱而空转。
"""
...
def _base_url(value: str | None = None) -> str:
    base = str(
        value if value is not None else getattr(_email_cfg, "MAILTM_API_BASE", "") or ""
    ).strip().rstrip("/")
    if not base:
...
def _request(method: str, path: str, *, token: str | None = None, json_body: dict | None = None) -> requests.Response:
    ...
    try:
        ...
        raise MailTmError(f"Mail.tm 请求失败 ({method} {path}): {type(exc).__name__}: {exc}") from exc


def _random_local(length: int | None = None) -> str:
    n = int(length if length is not None else getattr(_email_cfg, "MAILTM_RANDOM_LOCAL_LENGTH", 12) or 12)
    n = max(6, min(32, n))
...
def list_domains(force: bool = False) -> list[str]:
    """从 Mail.tm /domains 拉取可用域名（带短缓存）。"""
    global _DOMAIN_CACHE
    now = time.time()
    if not force and _DOMAIN_CACHE and now - _DOMAIN_CACHE[0] < DOMAIN_CACHE_TTL:
        return list(_DOMAIN_CACHE[1])

    configured = getattr(_email_cfg, "MAILTM_DOMAINS", None) or []
    if isinstance(configured, str):
        configured = [x.strip() for x in configured.replace(";", ",").split(",") if x.strip()]
    fixed = [str(x).strip().lower() for x in configured if str(x).strip()]
    if fixed:
    ...
    if resp.status_code != 200:
        raise MailTmError(f"Mail.tm /domains 失败: HTTP {resp.status_code}; {resp.text[:160]}")
    try:
        payload = resp.json()
    except ValueError as exc:
        raise MailTmError("Mail.tm /domains 响应不是 JSON") from exc

    # 公共 API 返回裸数组；自建镜像可能返回 {"hydra:member": [...]}，两种都兼容。
    members = payload if isinstance(payload, list) else (payload.get("hydra:member") if isinstance(payload, dict) else None)
    domains: list[str] = []
    if isinstance(members, list):
        for item in members:
            ...
            if d and item.get("isActive", True) is not False:
                domains.append(d)
    if not domains:
        raise MailTmError(f"Mail.tm 未返回可用域名: {str(payload)[:200]}")
    _DOMAIN_CACHE = (now, domains)
    return list(domains)


def pick_account() -> MailTmAccount:
    """创建并缓存一个 Mail.tm 邮箱（POST /accounts + /token）。"""
        ...
        try:
            ...
            if create.status_code not in (200, 201):
                ...
                if create.status_code == 422 and "already" in create.text.lower():
                    continue
                raise MailTmError(f"Mail.tm 创建邮箱失败: HTTP {create.status_code}; {create.text[:200]}")

            tok_resp = _request("POST", "/token", json_body={"address": address, "password": password})
            if tok_resp.status_code != 200:
                raise MailTmError(f"Mail.tm 登录取 token 失败: HTTP {tok_resp.status_code}; {tok_resp.text[:200]}")
            try:
                tok_payload = tok_resp.json()
            except ValueError as exc:
                raise MailTmError("Mail.tm /token 响应不是 JSON") from exc
            token = str((tok_payload or {}).get("token") or "").strip()
            if not token:
                raise MailTmError(f"Mail.tm /token 缺少 token: {str(tok_payload)[:160]}")

            account = MailTmAccount(email=address, password=password, token=token, domain=domain)
            _CONTEXT_CACHE[_cache_key(address)] = account
            logger.info("[MailTm] 已生成兜底邮箱: %s (domain=%s)", address, domain)
            return account
        except MailTmError as exc:
            last_exc = exc
            continue
    raise MailTmError(f"Mail.tm 创建邮箱多次失败: last={last_exc}")
...
def release_account(email: str, status: str = "available", note: str | None = None) -> None:
    ...
    logger.info("[MailTm] 已释放兜底邮箱: %s（status=%s, note=%s）", email, status, note or "")


def _ensure_token(account: MailTmAccount) -> str:
    if account.token:
        return account.token
    tok_resp = _request("POST", "/token", json_body={"address": account.email, "password": account.password})
    if tok_resp.status_code != 200:
        raise MailTmError(f"Mail.tm 刷新 token 失败: HTTP {tok_resp.status_code}; {tok_resp.text[:160]}")
    token = str((tok_resp.json() or {}).get("token") or "").strip()
    if not token:
        raise MailTmError("Mail.tm 刷新 token 为空")
    account.token = token
    return token
...
def _list_messages(token: str) -> list[dict]:
    resp = _request("GET", "/messages?page=1", token=token)
    if resp.status_code == 401:
        raise MailTmError("Mail.tm token 失效 (401)")
    if resp.status_code != 200:
        raise MailTmError(f"Mail.tm /messages 失败: HTTP {resp.status_code}; {resp.text[:160]}")
    try:
        payload = resp.json()
    except ValueError as exc:
        raise MailTmError("Mail.tm /messages 不是 JSON") from exc
    # 公共 API 返回裸数组；自建镜像可能返回 {"hydra:member": [...]}，两种都兼容。
    members = payload if isinstance(payload, list) else (payload.get("hydra:member") if isinstance(payload, dict) else None)
...
def _get_message(token: str, message_id: str) -> dict:
    ...
    if resp.status_code != 200:
        raise MailTmError(f"Mail.tm 消息详情失败: HTTP {resp.status_code}; {resp.text[:160]}")
    try:
        payload = resp.json()
    except ValueError as exc:
        raise MailTmError("Mail.tm 消息详情不是 JSON") from exc
...
def fetch_latest_otp(
    ...
    """轮询 Mail.tm 收件箱，返回领取时间后最新 OpenAI 六位验证码。"""
    target = str(email or "").strip()
    if not target:
        raise MailTmError("Mail.tm 取码缺少邮箱地址")

    account = get_account_context(target)
    if account is None:
        raise MailTmError(f"Mail.tm 进程内无该邮箱上下文: {target}（需由 pick_account 创建）")
    ...
    logger.info("[MailTm] 开始轮询兜底邮箱 %s，最长 %ss", target, wait_seconds)
        ...
        try:
            ...
            for summary in sortable:
                ...
                if best_otp is None or is_newer_message or is_updated_code:
                    ...
                    logger.info("[MailTm] 锁定 OTP 候选，等待 %ss 确认", settle)
    ...
    if best_otp:
        return best_otp
    raise MailTmError(f"等待 Mail.tm 验证码超时: {target}; {last_error}")
